...
if __name__ == '__main__':
    ...
    create_vocab(np.concatenate((train_samples, validation_samples)))  # find all distinct words from train and validation files
    ...
    # Naive Bayes model with Multinomial distribution
    model = MultinomialNB() 
    max_accuracy = 0
    best_percentage = 0
        
    for percentage in features_percentage:
        vect = CountVectorizer(lowercase=False, max_features=int(percentage*len(vocab)))
    ...
    vect = CountVectorizer(lowercase=False, max_features=int(best_percentage*len(vocab)))    
