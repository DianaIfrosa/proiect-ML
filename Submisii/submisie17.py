...
if __name__ == '__main__':
   ...
   # try n-grams on chars (find the best n between [4, 6])
    n_options = [x for x in range(4,7)]
    ...
    for n in n_options:
        vect = CountVectorizer(analyzer='char', lowercase=False, ngram_range=(n,n))

        train_data = vect.fit_transform(train_samples)
        validation_data = vect.transform(validation_samples)

        for hidden_layer_sizes_param in [(32, 64, 128,256), (64,128), (64,128,256)]:
    ...
    best_vect = CountVectorizer(analyzer='char', lowercase=False, ngram_range=(best_n,best_n))
