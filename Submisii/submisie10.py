...
if __name__ == '__main__':
    ...
    # try n-gramsfor words (find the best n between [2, 7]) - upperbound for word n-gram
    n_options = [x for x in range(2, 8)]
    ...
    for n in n_options:
        for preprocessing in [True, False]:
            vect = TfidfVectorizer(analyzer='word', lowercase=preprocessing, ngram_range=(1, n)) # use this representation in order to mark the most frequent words
            ...
            if accuracy > max_accuracy: # save the best parameters
                ...
                f = open("./accuracy-NB-tfidf.txt", "a", encoding='utf8')
                f.write(
                    "Accuracy for " + str(n) + "length: " + str(accuracy) + " preprocessing: " + str(preprocessing) + "\n")
                f.close()
    ...
    f.write("Best accuracy: " + str(max_accuracy) + ", best n: " + str(best_n) + ", lowercase: " + str(
        best_preprocessing) + "\n")
    f.close()

    vect = TfidfVectorizer(analyzer='word', lowercase=best_preprocessing, ngram_range=(1, best_n))
    ...
    f = open("./test_labels_NB-tfidf.txt", "w", encoding='utf8')
    f.write("id,label")
